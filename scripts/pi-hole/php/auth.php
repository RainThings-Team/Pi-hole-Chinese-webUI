...
function check_csrf($token) {
    ...
    if(!isset($_SESSION['token'])) {
        log_and_die("会话已过期！ 请在Pi-hole仪表板上重新登录。");
    }

    if(empty($token)) {
        log_and_die("空令牌！ 检查系统上是否启用了cookie。");
    }

    if(!hash_equals($_SESSION['token'], $token)) {
        log_and_die("令牌错误！ 请在Pi-hole仪表板上重新尝试登录。");
    }
}
...
function list_verify($type) {
    ...
    elseif(isset($_POST['pw']))
    {
        ...
        {
            log_and_die("密码错误  - ".htmlspecialchars($type)."listing of ".htmlspecialchars($_POST['domain'])." not permitted");
        }
    }
    else
    {
